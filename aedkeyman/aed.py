...
class ArborEdgeDefense(object):
    ...
    def __init__(
        self, hostname, token, creds, use_hsm=False, disable_cert_verify=False
    ):
        """
    ...
    def import_key(self, name, priv, cert=None):
        """Import a key."""
        body = {"label": name, "privateKey": priv, "certificate": cert}
    ...
    def list_keys(self):
        ...
        for item in res.json():
            ...
            key = {"name": item["label"], "type": item["type"]}
            if "public" in item:
        ...
        try:
            if method == "POST":
                result = requests.post(
                    url=url, headers=headers, json=data, verify=self.verify
                )
            elif method == "GET":
                result = requests.get(
                    url=url, headers=headers, params=data, verify=self.verify
                )
            elif method == "DELETE":
                result = requests.delete(
                    url=url, headers=headers, verify=self.verify
                )
