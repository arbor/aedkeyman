...
BEARER_TOKEN = (
    "25mjpor8S9igwuXhFi8UuWqb-O54cO1cZVG5r_BRIryMF7JUu7oXdV5ImI"
    + "F88UyEkURTEKp7Xi-c9dO2gC7r4w"
)

API_KEY = (
    "ZmYxNmQzZTctNDgxMS00OTNmLWE1MDEtOWUxZDFlYzkzYjljOmVIamhBUU"
    + "1Xd3NzcFBDMTNDZ3hUVEdhQTNZeEhabzVvcFl2UVRkM1FjRFJHRzJEQXVO"
    + "YWlJSXFrU21yTTNuTjZaeU90UDlMQnc1aWs4NjhtdTRTcm5B"
)

test_rsa_pub = (
    "MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAslLVLpYIGaPN23"
...
test_rsa_priv = (
    "MIIEowIBAAKCAQEAslLVLpYIGaPN23n7JGXwPwR4U1qRBXnoTOEGYH2EDm"
...
class SmartKeyTestCase(unittest.TestCase):
    ...
    def test_01_init_with_apikey_no_tokenfile(self, mopen, mrequest):
        ...
        mrequest.assert_called_with(
            headers=headers, method="POST", url=BASE_URL + "sys/v1/session/auth"
        )
    ...
    @patch("%s.open" % builtins_name)
    def test_04_user_auth_no_tokenfile_save(
        self, mopen, mosopen, moswrite, mosclose, mrequest
    ):
        mrequest.return_value = Mock()
        mrequest.return_value.status_code = requests.codes.ok
    ...
    def test_22_export_rsa_success(self, mpost):
        ...
        skey.token = BEARER_TOKEN
        headers = {"Authorization": "Bearer " + BEARER_TOKEN}
        req_data = {"kid": "fe0b726f-ae25-4939-a67b-382a4e7f35f7"}
        resp_data = {
            "acct_id": "bf6ea1a8-4e77-478f-a4e2-2524197614a8",
            "created_at": "20180809T152604Z",
            "creator": {"app": "ff16d3e7-4811-493f-a501-9e1d1ec93b9c"},
        ...
        mpost.assert_called_with(
            BASE_URL + "crypto/v1/keys/export",
            headers=headers,
            data=json.dumps(req_data),
        )
    ...
    def test_30_list_keys_success(self, mget):
        ...
        mget.assert_called_with(
            url=BASE_URL + "crypto/v1/keys", headers=headers, params=None
        )
        expected_rdata = resp_data
        self.assertEqual(actual_rdata, expected_rdata)
    ...
    def test_32_list_groups_success(self, mget):
        ...
        mget.assert_called_with(
            url=BASE_URL + "sys/v1/groups", headers=headers, params=None
        )
        expected_rdata = resp_data
        self.assertEqual(actual_rdata, expected_rdata)
    ...
    def test_40_delete_key_success(self, mdel):
        ...
        skey.delete_key(kid)
        mdel.assert_called_with(
            url=BASE_URL + "crypto/v1/keys/" + kid, headers=headers
        )
